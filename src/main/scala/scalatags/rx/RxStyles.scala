...
trait RxStyles extends RxDataConverter {
  ...
  implicit def RxDynamicStyleValue[A: StyleValue](implicit ctx: Ctx.Owner): RxStyleValue[A, Rx.Dynamic[A]] = new RxStyleValue

  final class RxStyleValue[A, R <: Rx[A]](implicit sv: StyleValue[A], ctx: Ctx.Owner) extends StyleValue[R] {
    override def apply(t: Element, s: Style, rx: R): Unit = rx.trigger(sv(t, s, _))
  }
...
trait RxDataConverter {
  ...
  implicit def RxDynamicPixelStyleValue[A](implicit ev: StyleValue[Rx.Dynamic[A]], ctx: Ctx.Owner): RxPixelStyleValue[A, Rx.Dynamic[A]] =
    new RxPixelStyleValue(ev)
